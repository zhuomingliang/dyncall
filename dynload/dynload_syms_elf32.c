...
int dlSymsCount(DLSyms* pSyms)
{
  ...
  else if (pSyms->pGNUHash)
    return pSyms->pGNUHash->symndx;
}
...
const char* dlSymsName(DLSyms* pSyms, int index)
{
  ...
  if (pSyms->pGNUHash) {
    ...
    uint32_t* buckets = & ( (Elf_Addr*) ( &pSyms->pGNUHash[1] ) ) [pSyms->pGNUHash->maskwords ];
    uint32_t* chains  = & buckets[pSyms->pGNUHash->nbuckets];

    str_index = pSyms->pSymTab[ chains[index] ].st_name;
...
{
  if (pSyms->pGNUHash)
    return (void*) pSyms->pSymTab[ pSyms->pGNUHash->symndx + index ].st_value;
  else
